...
without [[file:guix-with-env.scm][guix-with-env.scm]] due my base configurations (see ~(my-guix base)~ and
~(my-guix home base)~) including services that make the load paths persistent.
